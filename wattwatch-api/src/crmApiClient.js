...
import { CrmCustomer, CrmTicket, User, sequelize } from './models/index.js';
...
async function deleteCustomerViaApi(email) {

  const url = `${CRM_API_URL}/api/customers/account`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('CRM account delete API failed:', err.error || res.status);
    ...
    console.warn('CRM account delete API failed:', e.message);
...
async function deleteCustomerFromDb(email) {
  ...
  try {
    ...
    if (customer) {
      ...
      if (tables.includes('crm_tickets')) {

        await CrmTicket.destroy({ where: { customerId: customer.id } });
    ...
    console.warn('CRM account DB delete failed:', e.message);
...
/** Remove customer from CRM + shared users table when app account is deleted. */

export async function deleteCustomerFromCrm(email) {
  ...
  if (await deleteCustomerViaApi(normalized)) ok = true;

  if (await deleteCustomerFromDb(normalized)) ok = true;
